# =========================================================================
# COMPARATIVE SIMULATION ENGINE: BSSN vs fCCZ4 (Modes 1, 2, 3) 
# Maximal Slicing Only
# =========================================================================
import numpy as np
import time
import os
...
# 3. MOTOR DE EVOLUÇÃO
# =========================================================================
def executar_simulacao(modo):
    print(f"\n---> Running Simulation | Formalism: {modo} | N = {p} | K1 = {k1} <---")
    
    # Initial Conditions (Dispersão padrão A0 = 0.05 no vácuo)
    ...
    b = np.ones(p + 1)
    Delta = np.zeros(p + 1)
    Lam = np.zeros(p + 1) # Variável Lambda da conexão original (usada nos Modos 1 e 2)
    Aa = np.zeros(p + 1)
    beta = np.zeros(p + 1)
    B = np.zeros(p + 1)
    Z = np.zeros(p + 1)
    ...
    def evaluate_rhs(alpha_val, K_val, chi_val, a_val, b_val, Delta_val, Aa_val, beta_val, B_val, Z_val, Theta_val, Lam_val, aplicar_filtro=False):
        ...
        # ---------------------------------------------------------
        # ISOLAMENTO MATEMÁTICO DOS MODOS
        # ---------------------------------------------------------
        if modo == 'BSSN':
            ...
            # Evolve Z e Theta. Lam atua como a conexao principal (Delta ignorado)
            ze_coef = Alalpha @ Z_val
            drZ = DralphaAl @ ze_coef
            conn_for_R = Lam_val
            dr_conn_for_R = drLam
        elif modo == 'Mode2':
            # Evolve Delta, Lam, Theta. Z é deduzido algebricamente
            conn_for_R = Delta_val
            dr_conn_for_R = drDelta
            Z_val = 0.5 * (Delta_val - Lam_val) * a_val
            drZ = 0.5 * (drDelta - drLam) * a_val + 0.5 * (Delta_val - Lam_val) * dra
        elif modo == 'Mode3':
            # Evolve Delta, Theta. Lam é puramente geométrica
            conn_for_R = Delta_val
            dr_conn_for_R = drDelta
            Z_val = 0.5 * (Delta_val - L_geom) * a_val
            drZ = 0.5 * (drDelta - drL_geom) * a_val + 0.5 * (Delta_val - L_geom) * dra

        Zc = chi_val**(4.0 / nc) * Z_val / a_val
        divZ = chi_val**(4.0 / nc) * drZ / a_val + (-0.5 * dra / a_val + drb / b_val + 2.0 / rcol - (2.0 / nc) * drchi / chi_val) * Zc

        # ---------------------------------------------------------
        # MAXIMAL SLICING (APENAS)
        # ---------------------------------------------------------
        ...
        if modo == 'BSSN':
            ...
            dLam_dt = adveccao_conexao(Lam_val, drLam) + 2.0 * alpha_val * (Aa_val * Lam_val - 3.0 * Aa_val / b_val / rcol) + fonte_conexao
            dDelta_dt = adveccao_conexao(Delta_val, drDelta) + 2.0 * alpha_val * (Aa_val * Lam_val - 3.0 * Aa_val / b_val / rcol) + fonte_conexao \
                        + 2.0 * (alpha_val * drTheta - Theta_val * dralpha) / a_val - (4.0 / 3.0) * alpha_val * K_val * Z_val / a_val - 2.0 * k1 * alpha_val * Z_val / a_val
            dB_dt = dDelta_dt - eta0 * B_val
            dAa_dt = beta_val * drAa - TrLapalpha + alpha_val * TrRicci + alpha_val * K_val * Aa_val - 2.0 * alpha_val * Theta_val * Aa_val \
                     + 2.0 * alpha_val * (chi_val**(4.0 / nc) * drZ / a_val - (0.5 * dra / a_val - (2.0 / nc) * drchi / chi_val) * Zc - (1.0 / 3.0) * divZ)
            ...
            dLam_dt = np.zeros_like(Lam_val)
            dDelta_dt = adveccao_conexao(Delta_val, drDelta) + 2.0 * alpha_val * (Aa_val * L_geom - 3.0 * Aa_val / b_val / rcol) + fonte_conexao \
                        + 2.0 * (alpha_val * drTheta - Theta_val * dralpha) / a_val - (4.0 / 3.0) * alpha_val * K_val * Z_val / a_val - 2.0 * k1 * alpha_val * Z_val / a_val
            dB_dt = dDelta_dt - eta0 * B_val
            dAa_dt = beta_val * drAa - TrLapalpha + alpha_val * TrRicci + alpha_val * K_val * Aa_val - 2.0 * alpha_val * Theta_val * Aa_val \
                     + 2.0 * alpha_val * (chi_val**(4.0 / nc) * drZ / a_val - (0.5 * dra / a_val - (2.0 / nc) * drchi / chi_val) * Zc - (1.0 / 3.0) * divZ)
    ...
    while t <= tf:
        ...
        if niter % idump == 0:
            ...
            # Garantir que usamos a conexão correta no Cálculo de Rq dependendo do Modo
            if modo == 'Mode1':
            ...
            if np.isnan(L2HC) or L2HC > 1.0:
                print(f"     [ALERTA] Violação de restrição explodiu (L2HC = {L2HC:.2e}) em t={t:.2f}M! Avançando para próximo modo.")
                break
...
# =========================================================================
# 4. VARREDURA DOS MODOS E PLOTAGEM
# =========================================================================
modos_estudo = ['BSSN', 'Mode1', 'Mode2', 'Mode3']
resultados = {}

for m in modos_estudo:
    t_dados, hc_dados = executar_simulacao(m)
    resultados[m] = {'t': t_dados, 'hc': hc_dados}
...
cores_e_estilos = {
    'BSSN': ('k--', 'Pure BSSN', 2.0),
    'Mode1': ('r-', 'fCCZ4 Mode 1 (Evolves Z & $\\bar{\Lambda}$)', 1.5),
    'Mode2': ('g-', 'fCCZ4 Mode 2 (Evolves $\\tilde{\Delta}$ & $\\bar{\Lambda}$)', 1.5),
    'Mode3': ('b-', 'fCCZ4 Mode 3 (Standard fCCZ4/Z4c)', 2.5)
}

for m in modos_estudo:
    plt.plot(resultados[m]['t'], np.log10(resultados[m]['hc']), 
             cores_e_estilos[m][0], label=cores_e_estilos[m][1], lw=cores_e_estilos[m][2])

plt.title('Evolution of Hamiltonian Constraint Violation ($L_2$)\nMaximal Slicing | N = 200', fontweight='bold', fontsize=14)
...
nome_grafico = os.path.join(pasta_resultados, 'Comparison_Modes_FCCZ4.png')
plt.tight_layout()
plt.savefig(nome_grafico, dpi=300)
plt.show()

print(f"\nGráfico gravado com sucesso em: {nome_grafico}")
