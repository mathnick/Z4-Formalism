...
def configurar_base_unica(L0, N):
    # j vai de 1 até N+1. O cosseno nunca atinge -1 ou 1.
    # O domínio fica estritamente x in (-1, 1), excluindo r=0 e r=infinito!
    j = np.arange(1, N + 2)
    ...
    # --- O Filtro ERFC de H.P. de Oliveira (Traduzido para Python) ---
...
def criar_condicoes_iniciais_fccz4(b, M=1.0):
    ...
    # Subtraímos 1.0 de chi e alpha para evoluir como perturbação
...
# =========================================================================
# 3. EVOLUÇÃO fCCZ4 (SUBSTITUIÇÃO ANALÍTICA DO FATOR CONFORME)
# =========================================================================
def calcular_taxas_fccz4(state, kappa1, kappa2, eta_param, b):
    c_a, c_b, c_chi, c_K, c_Aa, c_Theta, c_Lambda, c_alpha, c_beta, c_B = state
    
    # Filtro ERFC nos modos
    c_a *= b['filtro']; c_b *= b['filtro']; c_chi *= b['filtro']
    c_K *= b['filtro']; c_Aa *= b['filtro']; c_Theta *= b['filtro']
    c_Lambda *= b['filtro']; c_alpha *= b['filtro']; c_beta *= b['filtro']; c_B *= b['filtro']
    ...
    # Escudos de Divisão
    chi_reg = np.maximum(chi, 1e-12)
    a_reg = np.maximum(a, 1e-12)
    b_reg = np.maximum(b, 1e-12)
    
    # Termos auxiliares analíticos para evitar o dX^2 / X^2
    chi_sq = chi_reg**2
    ...
    # Substituição 1: dX/X = 2 * dchi/chi
    ...
    # Substituição 2: Cancelamento analítico de ddX/X - dX^2/(2X^2) para ddchi/chi
    ...
    # Substituição 4: 1.5 * Aa * dX / X = 3.0 * Aa * dchi / chi
...
def executar_simulacao(L0, N, tf, h):
    ...
    print(f"fCCZ4 Definitivo (Domínio Único N={N}, dt={h}, eta={eta}, k1={k1})...")
    
    for i in range(int(tf/h)):
        # Adiciona 1.0 para recuperar o valor físico do Lapso na origem
        al_min = 1.0 + np.dot(s[7], b['psi'])[0]
        T_data.append(i*h); Alpha_data.append(al_min)
        
        if np.isnan(s).any() or np.max(np.abs(s)) > 1e11:
            print(f"\nCrash t={i*h:.4f}M.")
            r_grid = b['r']
            chi_val = 1.0 + np.dot(s[2], b['psi'])
            X_val = chi_val**2
            alpha_val = 1.0 + np.dot(s[7], b['psi'])
            shift_val = np.dot(s[8], b['psi'])
            
            plt.figure(figsize=(10,8))
            plt.subplot(3,1,1); plt.plot(r_grid, shift_val, 'r.-'); plt.ylabel("Shift")
            plt.subplot(3,1,2); plt.plot(r_grid, alpha_val, 'b.-'); plt.ylabel("Alpha")
            plt.subplot(3,1,3); plt.plot(r_grid, X_val, 'k.-'); plt.ylabel("X (via chi)"); plt.xlabel("r")
            plt.tight_layout(); plt.show()
            break
            
        # O passo RK4 já cuida do filtro silenciosamente lá dentro das taxas!
        s = passo_rk4(s, h, k1, k2, eta, b)
        
        if i % 10000 == 0:
            print(f"Tempo: {i*h:.2f}M | Alpha: {al_min:.5f}")
            
    return T_data, Alpha_data

# =========================================================================
# 5. DISPARO
# =========================================================================
T, A = executar_simulacao(L0=5.0, N=150, tf=15.0, h=0.00001)

if T[-1] > 14.5:
    print("\nSUCESSO! ESTADO ESTACIONÁRIO ALCANÇADO!")
    plt.figure(figsize=(10,6)); plt.plot(T, A, 'g', linewidth=2); plt.title("Trombeta Estável"); plt.xlabel("Tempo (M)"); plt.ylabel("Alpha na Origem"); plt.grid(True); plt.show()
