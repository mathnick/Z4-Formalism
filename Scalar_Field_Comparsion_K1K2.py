...
r0 = 2
sigma = 1
A0 = 0.05 # 0.05 (Dispersão) ou 0.09 (Colapso)
V = 0 # Potencial
...
def executar_simulacao_kappa(c0, a0, b0, theta0, z0, t_init, h, It, kappa1, kappa2):
    ...
    L2MC_data = [] # L2 Momentum Constraint - CORREÇÃO: ADICIONADA ESTA LINHA
    ...
    # CORREÇÃO: AGORA RETORNA 4 VALORES
...
cenarios_kappa = {
    ...
    r"$\kappa_1=1000, \kappa_2=0$": (1000, 0,'b--'),      # Azul, linha tracejada
    r"$\kappa_1=10, \kappa_2=-0.5$": (10, -0.5, 'r:'),       # Vermelho, linha pontilhada
    r"$\kappa_1=80, \kappa_2=0.5$": (80, 0.5, 'g-.')       # Verde, linha traço-ponto
}
...
print(f"Iniciando múltiplas simulações com A0 = {A0}. Iterações por rodada: {It}")

# 3. Loop de Execução
for label, (k1, k2, linestyle) in cenarios_kappa.items():
    # CORREÇÃO: AGORA DESEMPACOTANDO 4 VALORES
    Time_data, Alpha_data, L2HC_data, L2MC_data = executar_simulacao_kappa(
...
plt.title(r"Violação da Restrição Hamiltoniana $L_2$ para diferentes $\kappa$", fontsize=14)
...
# 6. PLOTAGEM FINAL (Restrição de Momento)
plt.figure(figsize=(12, 7)) 
for label, data in resultados_mc.items():
    # Usamos o log dos dados da Restrição
    plt.plot(data['Time'], data['L2MC'], data['Style'], label=label, linewidth=2)

plt.title(r"Violação da Restrição de Momento $L_2$ para diferentes $\kappa$", fontsize=14)
