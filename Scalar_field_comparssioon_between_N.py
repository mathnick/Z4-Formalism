...
# =========================================================================
# 3. EXECUÇÃO DA SIMULAÇÃO COM N VARIÁVEL E L0 FIXO
# =========================================================================

# 1. Definições dos Cenários (L0 fixo = 10, N variável)
L0_FIXO = 10
valores_N = [50, 100, 150, 200]

cenarios_N = {
    # Etiqueta: (L0_valor, N_valor, estilo_linha)
    r"$N=50$": (L0_FIXO, 50, 'b-'),        # Azul, linha cheia
    r"$N=100$": (L0_FIXO, 100, 'r--'),     # Vermelho, linha tracejada  
    r"$N=150$": (L0_FIXO, 150, 'g:'),      # Verde, linha pontilhada
    r"$N=200$": (L0_FIXO, 200, 'm-.')      # Magenta, linha traço-ponto
}
...
print(f"Iniciando múltiplas simulações com L0={L0_FIXO}, A0={A0_FIXO}, kappa1={KAPPA1_FIXO}, kappa2={KAPPA2_FIXO}")
print(f"Iterações por rodada: {It}")

# 4. Loop de Execução para diferentes valores de N
for label, (L0_valor, N_valor, linestyle) in cenarios_N.items():
...
# =========================================================================
# 4. PLOTAGEM FINAL - COMPARAÇÃO ENTRE DIFERENTES VALORES DE N
# =========================================================================
...
plt.title(f"Evolução do Lapse Central $\\alpha(t,0)$ para diferentes $N$\n" + 
          f"($L_0={L0_FIXO}$, $A_0={A0_FIXO}$, $\\kappa_1={KAPPA1_FIXO}$, $\\kappa_2={KAPPA2_FIXO}$)", fontsize=14)
plt.ylabel(r"$\alpha(t,0)$", fontsize=12)
plt.xlabel("t", fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)
plt.legend(title="Configurações $N$", fontsize=10)
plt.show() 
...
plt.title(f"Violação da Restrição Hamiltoniana $L_2$ para diferentes $N$\n" + 
          f"($L_0={L0_FIXO}$, $A_0={A0_FIXO}$, $\\kappa_1={KAPPA1_FIXO}$, $\\kappa_2={KAPPA2_FIXO}$)", fontsize=14)
...
plt.title(f"Violação da Restrição de Momento $L_2$ para diferentes $N$\n" + 
          f"($L_0={L0_FIXO}$, $A_0={A0_FIXO}$, $\\kappa_1={KAPPA1_FIXO}$, $\\kappa_2={KAPPA2_FIXO}$)", fontsize=14)
