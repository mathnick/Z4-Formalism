# =========================================================================
# COMPARATIVE SIMULATION ENGINE: BSSN vs fCCZ4 (Mode 1) 
# Maximal Slicing Only | Scanning Kappa_1
# =========================================================================
import numpy as np
import time
import os
...
def executar_simulacao(modo, k1):
    ...
    nome_arq = os.path.join(pasta_resultados, f"Erros_{modo}_k{k1}.txt")
...
# 1º Passo: Executar o BSSN Puro (só é necessário correr 1 vez)
t_bssn, hc_bssn = executar_simulacao('BSSN', k1=0.0)

# 2º Passo: Executar o fCCZ4 (Mode 1) para as várias Kappas
kappa_values = [20.0, 25.0, 30.0]
resultados_fccz4 = {}

for k in kappa_values:
    t_fccz4, hc_fccz4 = executar_simulacao('Mode1', k1=k)
    resultados_fccz4[k] = {'t': t_fccz4, 'hc': hc_fccz4}

print("\nGerando Gráfico Comparativo Único...")

# 3º Passo: Gerar um único gráfico com todas as curvas
plt.figure(figsize=(10, 7))

# Plota a linha de base (BSSN) mais espessa e a preto tracejado
plt.plot(t_bssn, np.log10(hc_bssn), 'k--', lw=2.5, label='Pure BSSN')

# Cores e estilos para diferenciar os diferentes kappas
cores_estilos = {
    20.0: ('r-', 2.0),
    25.0: ('g-', 2.0),
    30.0: ('b-', 2.0)
...
for k in kappa_values:
    estilo, espessura = cores_estilos[k]
    plt.plot(resultados_fccz4[k]['t'], np.log10(resultados_fccz4[k]['hc']), 
             estilo, lw=espessura, label=f'fCCZ4 Mode 1 ($\\kappa_1={k}$)')
...
nome_grafico = os.path.join(pasta_resultados, 'Comparison_BSSN_vs_Mode1_Kappas.png')
plt.tight_layout()
plt.savefig(nome_grafico, dpi=300)
plt.show()

print(f" -> Gráfico gravado com sucesso em: {nome_grafico}")
print("\nConcluído! Pode visualizar o gráfico consolidado.")
