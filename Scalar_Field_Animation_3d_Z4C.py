...
A0 = 0.05 # 0.08 -> disperses ; # 0.09-> disperses ;
...
# Configuração da Animação COM 2D E 3D
fig_anim = plt.figure(figsize=(12, 5))

# Subplot para 2D
ax1 = fig_anim.add_subplot(121)
ax1.set_xlim(0, 10)
ax1.set_ylim(-2, 1.5)
line, = ax1.plot([], [], lw=2, color='b')
ax1.set_title(r"Evolução do Campo Escalar $\phi(t, r)$")
ax1.set_xlabel("r")
...
# Inicializar a superfície 3D vazia
surface = [ax2.plot_surface(np.zeros((10, 10)), np.zeros((10, 10)), np.zeros((10, 10)), 
                           rstride=5, cstride=5, cmap='magma', edgecolor='none')]

# Textos para exibir informações
initA0_text = ax1.text(0.02, 0.95, '', transform=ax1.transAxes)
time_text = ax1.text(0.02, 0.9, '', transform=ax1.transAxes)

# Preparar coordenadas para a superfície 3D
M = len(rplot)
...
def init_anim():
    line.set_data([], [])
    # Remover a superfície anterior
    for surf in surface:
        surf.remove()
    surface[0] = ax2.plot_surface(xn, yn, zn, rstride=5, cstride=5, cmap='magma', 
                                edgecolor='none', alpha=0.8)
    
    initA0_text.set_text(f"$A_0 = {A0}$")
    time_text.set_text("Time = 0.00")
    return line, surface[0], initA0_text, time_text
...
def animate_func(i):
    ...
    # Criar a superfície 3D atualizada
...
anim = FuncAnimation(fig_anim, animate_func, init_func=init_anim,
                     frames=TOTAL_FRAMES_SALVOS, interval=(1000/FPS), blit=False)  # blit=False para 3D

# Ajustar layout
plt.tight_layout()

# Salvar a animação
print(f"Salvando {TOTAL_FRAMES_SALVOS} frames em MP4 com visualização 2D+3D...")

writer = mpl.animation.FFMpegWriter(fps=FPS, metadata=dict(artist='Z4c Solver'), bitrate=1800)
anim.save("animation_MS_collapse_2D_3D_A_005.mp4", writer=writer)

print("Animação 2D+3D salva com sucesso em 'animation_MS_collapse_2D_3D.mp4'")
