# =========================================================================
# CONVERGENCE SIMULATION ENGINE: BSSN vs fCCZ4 (Mode 1) 
# L0 = 5.0 | k1 = 25.0 | Filtering Fixed
# =========================================================================
import numpy as np
import os
...
pasta_resultados = os.path.join(pasta_atual, "Resultados_Convergencia")
...
N_values = [100, 125, 160, 180, 200]

def carregar_matriz(nome, p):
    caminho = os.path.join(pasta_atual, f"{nome}_{p}.txt")
    if os.path.exists(caminho): return np.loadtxt(caminho)
    raise FileNotFoundError(f"Matriz {nome}_{p}.txt nao encontrada! Rode o gerador limpo.")

def executar_simulacao(modo, gauge, p):
    print(f" -> Rodando: {modo} | {gauge} Slicing | N = {p}")
    
    base_rcol = carregar_matriz('rcol', p).flatten()
    base_rqcol = carregar_matriz('rqcol', p).flatten()
    wcolq = carregar_matriz('Wcolq', p).flatten()
    AlphaAl = carregar_matriz('AlphaAl', p)
    Alalpha = carregar_matriz('Alalpha', p)
    Faa = np.copy(Alalpha)
    base_DralphaAl = carregar_matriz('DralphaAl', p)
    base_DrralphaAl = carregar_matriz('DrralphaAl', p)
    ChiqC = carregar_matriz('PhiqA', p)
    base_DrchiqC = carregar_matriz('DrphiqA', p)
    base_DrrchiqC = carregar_matriz('DrrphiqA', p)

    Nq = int((3.0 / 2.0) * p)
    eta1_vec = np.arange(1, p + 2) / (p + 1)
    # Filtro espectral clássico
    ...
    def evaluate_rhs(alpha_val, K_val, chi_val, a_val, b_val, Delta_val, Aa_val, beta_val, B_val, Z_val, Theta_val, Lam_val, aplicar_filtro=False):
        # A MÁGICA ESPECTRAL: Filtrar antes de derivar
        ...
        if gauge == 'Maximal':
            ...
            Lapalpha = chi_val**(4.0 / nc) / a_val * (drralpha - dralpha * Gamma_term)
    ...
    while t_val <= tf:
        ...
        if niter % idump == 0:
            ...
            if np.isnan(L2HC) or L2HC > 10.0:
                print("       [CRASH] Simulação estourou.")
                break
    ...
    # RMS final
    rms = np.sqrt(np.mean(np.array(L2HC_data)**2)) if len(L2HC_data) > 0 else np.nan
    print(f"       -> RMS Final = {rms:.2e}")
    return rms

print("\n=======================================================")
print(" INICIANDO TESTE DE CONVERGÊNCIA (MATRIZES LIMPAS L0=1)")
print("=======================================================")
...
for gauge in ['Maximal', 'Log']:
    print(f"\n--- AVALIANDO CALIBRE: {gauge} SLICING ---")
    for modo in ['BSSN', 'Mode1']:
        for N_val in N_values:
            rms_val = executar_simulacao(modo, gauge, N_val)
            resultados[gauge][modo].append(rms_val)

print("\nGerando Gráficos de Convergência...")

plt.figure(figsize=(8, 6))
plt.semilogy(N_values, resultados['Maximal']['BSSN'], 'k-o', linewidth=2.0, markersize=8, markerfacecolor='none', markeredgewidth=2, label='BSSN')
plt.semilogy(N_values, resultados['Maximal']['Mode1'], 'r-s', linewidth=2.0, markersize=8, markerfacecolor='none', markeredgewidth=2, label=f'fCCZ4 Mode 1 ($\\kappa_1={k1}$)')
plt.title('Convergence: Algebraic Decay of $L_2(HC)_{RMS}$\nMaximal Slicing', fontweight='bold', fontsize=14)
plt.xlabel('Number of Spectral Points ($N$)', fontsize=13)
plt.ylabel(r'$L_2(HC)_{RMS}$', fontsize=13)
plt.xticks(N_values)
plt.grid(True, which="both", linestyle='--', alpha=0.7)
plt.legend(loc='upper right', fontsize=12)
plt.tight_layout()
plt.savefig(os.path.join(pasta_resultados, 'Convergence_Maximal.png'), dpi=300)
plt.show()

plt.figure(figsize=(8, 6))
plt.semilogy(N_values, resultados['Log']['BSSN'], 'k-o', linewidth=2.0, markersize=8, markerfacecolor='none', markeredgewidth=2, label='BSSN')
plt.semilogy(N_values, resultados['Log']['Mode1'], 'b-^', linewidth=2.0, markersize=8, markerfacecolor='none', markeredgewidth=2, label=f'fCCZ4 Mode 1 ($\\kappa_1={k1}$)')
plt.title('Convergence: Algebraic Decay of $L_2(HC)_{RMS}$\n1+log Slicing', fontweight='bold', fontsize=14)
plt.xlabel('Number of Spectral Points ($N$)', fontsize=13)
plt.ylabel(r'$L_2(HC)_{RMS}$', fontsize=13)
plt.xticks(N_values)
plt.grid(True, which="both", linestyle='--', alpha=0.7)
plt.legend(loc='upper right', fontsize=12)
plt.tight_layout()
plt.savefig(os.path.join(pasta_resultados, 'Convergence_Log.png'), dpi=300)
plt.show()
