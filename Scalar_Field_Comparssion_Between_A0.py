...
# =========================================================================
# 1. FUNÇÃO PARA CRIAR CONDIÇÕES INICIAIS PARA DIFERENTES A0
# =========================================================================

def criar_condicoes_iniciais(A0_valor):
    """
    Cria condições iniciais para um valor específico de A0
    """
    r0 = 2
    sigma = 1
    V = 0 # Potencial
...
def executar_simulacao_A0(c0, a0, b0, theta0, z0, t_init, h, It, kappa1, kappa2):
    """
    Executa a simulação completa para condições iniciais específicas
...
# =========================================================================
# 3. EXECUÇÃO DA SIMULAÇÃO COM MÚLTIPLOS VALORES DE A0
# =========================================================================

# 1. Definições dos Cenários de A0
cenarios_A0 = {
    # Etiqueta: (A0_valor, estilo_linha)
    r"$A_0=0.02$": (0.02, 'b-'),      # Azul, linha cheia
    r"$A_0=0.05$": (0.05, 'r--'),     # Vermelho, linha tracejada  
    r"$A_0=0.08$": (0.09, 'g:'),      # Verde, linha pontilhada
    r"$A_0=0.12$": (0.12, 'm-.'),     # Magenta, linha traço-ponto
    r"$A_0=0.15$": (0.15, 'c--')      # Ciano, linha tracejada
}

# 2. Parâmetros FIXOS de kappa (agora constantes para todas as simulações)
KAPPA1_FIXO = 10
KAPPA2_FIXO = -0.5

# 3. Inicialização dos parâmetros fixos de simulação
h = 0.0002
tf = 8
It = int(tf/h)
t = 0.0 
...
print(f"Iniciando múltiplas simulações com kappa1={KAPPA1_FIXO}, kappa2={KAPPA2_FIXO}")
print(f"Iterações por rodada: {It}")

# 4. Loop de Execução para diferentes A0
for label, (A0_valor, linestyle) in cenarios_A0.items():
    print(f"\nCriando condições iniciais para {label}...")
    
    # Criar condições iniciais específicas para este A0
    c0_temp, a0_temp, b0_temp, theta0_temp, z0_temp = criar_condicoes_iniciais(A0_valor)
    
    print(f"Executando simulação para {label}...")
    Time_data, Alpha_data, L2HC_data, L2MC_data = executar_simulacao_A0(
        c0_temp, a0_temp, b0_temp, theta0_temp, z0_temp, t, h, It, KAPPA1_FIXO, KAPPA2_FIXO
    )
...
# =========================================================================
# 4. PLOTAGEM FINAL - COMPARAÇÃO ENTRE DIFERENTES A0
# =========================================================================
...
plt.title(f"Evolução do Lapse Central $\\alpha(t,0)$ para diferentes $A_0$\n" + 
          f"($\\kappa_1={KAPPA1_FIXO}$, $\\kappa_2={KAPPA2_FIXO}$)", fontsize=14)
plt.ylabel(r"$\alpha(t,0)$", fontsize=12)
plt.xlabel("t", fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)
plt.legend(title="Amplitude Inicial", fontsize=10)
plt.show() 
...
plt.title(f"Violação da Restrição Hamiltoniana $L_2$ para diferentes $A_0$\n" + 
          f"($\\kappa_1={KAPPA1_FIXO}$, $\\kappa_2={KAPPA2_FIXO}$)", fontsize=14)
...
plt.title(f"Violação da Restrição de Momento $L_2$ para diferentes $A_0$\n" + 
          f"($\\kappa_1={KAPPA1_FIXO}$, $\\kappa_2={KAPPA2_FIXO}$)", fontsize=14)
